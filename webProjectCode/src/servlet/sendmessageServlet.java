...
public class sendmessageServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Messages md = new Messages(); //实例化一个对象，组装属性
        md.setpictureUrl("1");
        md.sethavePicture(0);
                ...
                while (fileItems.hasNext()) {
                    ...
                    if (fileItem.isFormField()) {
                        ...
                        if(name.equals("Tousername")) {
                            md.settousername(values);
                        }else if(name.equals("mailWords")) {
                            md.setmessageWord(values);
                        }
        ...
        System.out.println("getdynamic_words"+md.getmessageWord());
        System.out.println("gethavapictue"+md.gethavePicture());
        if(md.gettousername().equals("")||md.gettousername()==null) {
            request.setAttribute("unsent", "不知道发送给谁");  //向request域中放置参数
            request.getRequestDispatcher("/userinfor.jsp").forward(request, response);
            return ;
        }
        if((md.getmessageWord().equals("")||md.getmessageWord()==null)&&md.gethavePicture()==0) {
            request.setAttribute("unsent", "不能发布空的信息");  //向request域中放置参数
            request.getRequestDispatcher("/userinfor.jsp").forward(request, response);
            return ;
        }
        ProcedureDao mpd = new ProcedureDaoImpl();
        if(mpd.sentMessages(md,nm)){
            request.setAttribute("sent", "发送成功");  //向request域中放置参数
        }else{
            request.setAttribute("unsent", "发送失败");  //向request域中放置参数
        }
        request.getRequestDispatcher("/userinfor.jsp").forward(request, response);
    }
}
